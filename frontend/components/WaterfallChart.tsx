...
export default function WaterfallChart({ requests }: WaterfallChartProps) {
  ...
  // Determine timing limits for the relative chart width
  const timingLimits = useMemo(() => {
    if (requests.length === 0) return { maxTime: 1000 };
    
    // Find the latest end time relative to page load start
    let maxTime = 100;
    requests.forEach(r => {
  ...
  const filteredRequests = useMemo(() => {
    return requests.filter(r => {
      ...
      const matchesFilter = activeFilter === "all" 
        ...
        || (activeFilter === "xhr" && r.resourceType === "fetch") // group fetch/xhr
          ...
          <div className="max-h-[400px] overflow-y-auto divide-y divide-zinc-800/50">
            {filteredRequests.length === 0 ? (
              ...
              filteredRequests.map((req, idx) => {
                ...
                const durationPct = Math.max((req.duration / timingLimits.maxTime) * 100, 1.5); // min width 1.5%
